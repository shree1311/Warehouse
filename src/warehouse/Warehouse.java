...
public class Warehouse {
    ...
    private void addToEnd(int id, String name, int stock, int day, int demand) {
        int sector = (id % 10);
        Product toAdd = new Product(id, name, stock, day, demand);

        sectors[sector].add(toAdd);
    }
    ...
    private void fixHeap(int id) {
        int sector = (id % 10);
        int size = sectors[sector].getSize();
        for (int i = size; i > 1; i--) {
            sectors[sector].swim(i);
        }
    }
    ...
    public void restockProduct(int id, int amount) {
        ...
        for (int i = 1; i <= size; i++) {
            Product toCompare = sectors[sector].get(i);
    ...
    public void deleteProduct(int id) {
        int sector = (id % 10);
        int size = sectors[sector].getSize();
        System.out.println(sector + " has " + size + " items " + sectors[sector]);
        for (int i = 1; i <= size; i++) {
            Product toCompare = sectors[sector].get(i);
            if (toCompare != null) {
                if (toCompare.getId() == id) {
                    sectors[sector].swap(i, size);
                    sectors[sector].deleteLast();
    ...
    public void purchaseProduct(int id, int day, int amount) {
        ...
        for (int i = 1; i <= size; i++) {
            Product toCompare = sectors[sector].get(i);
            if (toCompare.getId() == id) {
                if (toCompare.getStock() > amount) {
                    toCompare.setLastPurchaseDay(day);
                    toCompare.setStock(toCompare.getStock() - amount);
                    toCompare.updateDemand(amount);
                    sectors[sector].sink(i);
                }
            }
        }
